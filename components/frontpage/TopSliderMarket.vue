<template>
  ...
  <div v-swiper:mySwiper="swiperOption">
    <div class="swiper-wrapper">
      <div v-for="(game, idx) in gamesTopSlider" :key="idx" class="swiper-slide game-thumbnail" :class="`comingsoon-${game.comingSoon}`">
...
import allGames from '../../static/data/games-markets.json'
  export default {
...
//      return allGames.filter(val => (val.custom !== true && val.branded !== true))
      return allGames.filter(val => (val.custom !== true && val.comingSoon !== true && val.markets && val.markets.includes(this.id)))
