      ...
      <div v-if="currentGame.comingSoon === false" class="screenshots-carousel__wrap">
        ...
        <div v-swiper:mySwiper3="swiperOption">
          <div id="lightgallery" class="swiper-wrapper">
            ...
            <div v-for="(feature, idx) in filteredFeatures" :key="idx"  class="swiper-slide game-thumbnail slide-static" :data-src="`/images/${feature.imgUrl}`" :data-sub-html="`${feature.name}`" >
...
export default {
 ...
 computed: {
  ...
  filteredFeatures() {
    // Filter out features where imgUrl is null and slice from the second element
    return this.currentGame.features.slice(1).filter(feature => feature.imgUrl !== null);
  },
